...
namespace Proyecto2.Graficos
{
    class Dibujar
    {
        public Dibujo drawForm = new Dibujo();

        public void dibujo(List<Figura> lista, string nombre)
        {
            drawForm.Show();
            drawForm.Size = new Size(800, 600);
            drawForm.Text = nombre;

            foreach(Figura figura in lista)
            {
                switch (figura.tipo){
                    case "circulo":
                        DrawCircle(figura.radio, figura.posx, figura.posy, figura.solida, figura.color);
                        break;
                    case "triangulo":
                        DrawTriangle(figura.vx1, figura.vy1, figura.vx2, figura.vy2, figura.vx3, figura.vy3, figura.solida, figura.color);
                        break;
                    case "cuadrado":
                        DrawSquare(figura.posx, figura.posy, figura.width, figura.height, figura.solida, figura.color);
                        break;
                    case "linea":
                        DrawLine(figura.posx, figura.posy, figura.posxf, figura.posyf, figura.grosor, figura.color);
                        break;
